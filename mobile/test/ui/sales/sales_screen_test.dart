...
class FakeProductRepository implements ProductRepository {
  ...
  @override
  Future<Product?> getById(int id) async => _products.first;
  @override
  Future<bool> update(Product product) async => true;
  @override
...
void main() {
  ...
  testWidgets('tap produk -> tambah ke cart & total terupdate', (tester) async {
    ...
    expect(
      find.text('Rp 3000'),
      findsWidgets,
    ); // Rp 3000 product + Rp 3000 in bottom bar
  ...
  testWidgets(
    'pilih Kasbon + pilih customer -> debtRepository.createDebt dipanggil',
    (tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
  ...
  testWidgets(
    'pilih Kasbon, dialog kosong, tap "Tambah Customer" -> CustomerFormScreen muncul',
    (tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(
        find.widgetWithText(AppBar, 'Tambah Customer'),
        findsOneWidget,
      ); // title of CustomerFormScreen
  ...
  testWidgets(
    'SalesScreen shows [− qty +] when qty>0, + disabled at stock limit',
    (tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(
        find.descendant(of: find.byType(Card), matching: find.text('1')),
        findsOneWidget,
      );
      ...
      expect(
        find.descendant(of: find.byType(Card), matching: find.text('10')),
        findsOneWidget,
      );
  ...
  testWidgets('SalesScreen tap minus when qty=1 removes item from cart', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1080, 2400);
    tester.view.devicePixelRatio = 1.0;
  ...
  testWidgets('Menampilkan chip Semua + kategori unik dari produk', (
    tester,
  ) async {
    final localRepo = FakeProductRepository(
      products: [
        Product(
          id: 1,
          tenantId: 'tenant-1',
  ...
  testWidgets('Tap chip kategori memfilter grid produk', (tester) async {
    final localRepo = FakeProductRepository(
      products: [
        Product(
          id: 1,
    ...
    expect(find.text('Rokok A'), findsNothing);

    await tester.pumpWidget(Container());
  });

  testWidgets('Produk tanpa kategori masuk chip Lainnya', (tester) async {
    final localRepo = FakeProductRepository(
      products: [
        Product(
          id: 1,
    ...
    expect(find.text('Produk X'), findsOneWidget);

    await tester.pumpWidget(Container());
  });

  testWidgets('Tap chip Semua menampilkan kembali semua produk', (
    tester,
  ) async {
    final localRepo = FakeProductRepository(
      products: [
